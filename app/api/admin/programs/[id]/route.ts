  ...
  try {
    ...
    if (file && file.size > 0) {
      ...
      const { error: uploadError } = await supabaseAdmin.storage
        .from('uploads')
        .upload(`programs/${fileName}`, buffer, {
    ...
    return NextResponse.json({ ok: true, message: "Program item updated successfully" });
  } catch (err) {
    console.error("Admin Programs PUT Error:", err);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ ok: true, message: "Program item deleted successfully" });
  } catch (err) {
