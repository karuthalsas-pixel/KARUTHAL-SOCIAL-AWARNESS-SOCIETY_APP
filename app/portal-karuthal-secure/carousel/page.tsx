...
export default function AdminCarousel() {
  ...
  const [isEditing, setIsEditing] = useState<CarouselItem | null>(null);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // clear imageUrl if file is selected so it doesn't conflict visually, though the backend prioritizes file
    }
  };
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-display font-bold text-white">Home Carousel</h1>
          <p className="text-[#AFDDE5]/70 mt-1">Manage the fast-moving carousel at the bottom of the home page</p>
        </div>
      ...
      {(isAdding || isEditing) && (
        <div className="mb-8 p-6 bg-[#02393e]/50 border border-[#0FA4AF]/30 rounded-2xl">
          <h2 className="text-xl font-bold text-white mb-4">{isEditing ? "Edit Item" : "New Item"}</h2>
          <form onSubmit={handleSave} className="space-y-4">
            
            {/* Image Selection Area */}
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="col-span-full">
                <label className="block text-sm font-medium text-[#AFDDE5] mb-2">Carousel Image *</label>
                <div className="flex flex-col sm:flex-row gap-6 bg-[#001f22] p-4 rounded-xl border border-[#0FA4AF]/30 items-center">
                  ...
                  <div className="flex-1 space-y-4 w-full">
                    <div>
                      ...
                      <input type="file" accept="image/*" ref={fileInputRef} onChange={handleFileChange} className="hidden" id="carousel-upload" />
                      <label htmlFor="carousel-upload" className="inline-block bg-[#024950] hover:bg-[#0FA4AF]/40 border border-[#00F0FF]/40 text-[#AFDDE5] px-4 py-2 rounded-lg cursor-pointer transition-colors text-sm font-medium w-full text-center sm:w-auto">
                        Choose File...
                      </label>
                    </div>
              ...
              <div>
                <label className="block text-sm text-[#AFDDE5] mb-1">Title *</label>
                <input required value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full bg-[#001f22] border border-[#0FA4AF]/30 rounded-lg p-2.5 text-white" />
              </div>
            ...
            <div>
              ...
              <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} rows={2} className="w-full bg-[#001f22] border border-[#0FA4AF]/30 rounded-lg p-2.5 text-white" />
            </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {items.length === 0 ? (
          <div className="col-span-full p-8 text-center text-[#AFDDE5]/50 border border-[#0FA4AF]/20 border-dashed rounded-2xl">
            No carousel items found. Add some to get started!
          </div>
        ) : (
          items.map((item) => (
            <div key={item.id} className="bg-[#002b2e]/60 rounded-2xl border border-[#0FA4AF]/20 overflow-hidden flex flex-col group">
              <div className="h-48 w-full bg-[#001f22] relative overflow-hidden">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={item.imageUrl} alt={item.title} className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                <div className="absolute top-2 right-2 bg-black/50 backdrop-blur-md px-2 py-1 rounded-md text-xs font-mono text-white border border-white/10">
                  Order: {item.order}
                </div>
              </div>
      ...
      <DeleteConfirmModal 
        ...
        message="Are you sure you want to delete this carousel item?"
      />
    </div>
  );
}
